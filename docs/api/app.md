...
* `name` string - You can request the following paths by the name:
  ...
  * `userCache` The directory for storing your app's cache files, which
    by default is the `cache` directory appended with your app's name. HTTP disk
    cache are written into this directory.
  * `sessionData` The directory for storing data generated by `Session` except for
    HTTP disk cache, such as localStorage, cookies, downloaded dictionaries,
    network state, devtools files. By default this points to `userData`.
...
By default, web pages' HTTP caches will be written to the `userCache` directory,
and other files like cookies and localStorage will be stored under the
